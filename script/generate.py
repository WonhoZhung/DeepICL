...
class Generator(DataProcessor):
    ...
    def _get_random_transformation(
            self,
            translation_coeff=0.2,
            rotation_coeff=PI/90
            ):
    ...
    def _pocket_coeff_scheduler(
        ...
        self.pocket_coeff = self.pocket_coeff_max * \
                torch.exp((self.pocket_coeff_thr - mean_dist) * self.pocket_coeff_beta)
        self.pocket_coeff = torch.clamp(self.pocket_coeff, max=self.pocket_coeff_max)

    def _make_input_list(
            self,
            ):
        input_list = [(i, j) for i in range(len(self.keys)) \
                for j in range(self.args.num_sample)]
        return input_list

    @torch.no_grad()
    def _get_input_from_data(
            self,
            data
            ):
        r"""
        ...
        ligand_type, ligand_coord, pocket_type, pocket_coord, _, ligand_prop, scaff_prop, \
        _, scaff_n, _, _, pocket_prop = data
        ...
        if self.add_noise:
            t, R = self._get_random_transformation(self.translation_coeff, \
                    self.rotation_coeff)
            p_R = p_R @ R + t
            p_dict.update({
                "translation": t,
    ...
    def _generate_molecule(
        ...
        while i < self.max_num_atom:
            ...
            if i > 0 and torch.sum(avail) == 0: break
            if i == 0:
                f_R = self.o_R
            else:
            ...
            p_ind = torch.topk(torch.cdist(f_R, p_dict["p_R"]), k=self.args.k, \
                    dim=-1, largest=False)[1][0]
            ...
            next_type_lp_log_prob = self.pocket_coeff * \
                    self.model.next_type_lp(data, "pocket").squeeze(0)
            next_type_log_prob = next_type_ll_log_prob + next_type_lp_log_prob
            next_type_log_prob -= torch.logsumexp(next_type_log_prob, dim=-1, keepdim=True)
            if self.t1 == 0:
            ...
            if i == 0:
                grid = self._make_grid(f_R, (0., self.radial_limits[-1])) # TODO
            else: 
                grid = self._make_grid(f_R, self.radial_limits)

            next_dist_ll_log_prob = self.ligand_coeff * \
                    self.model.next_dist_ll(data, next_type, "ligand")
            next_dist_lp_log_prob = self.pocket_coeff * \
                    self.model.next_dist_lp(data, next_type, "pocket")
            next_dist_log_prob = torch.cat([next_dist_lp_log_prob, next_dist_ll_log_prob], 0)
            next_dist_log_prob -= torch.logsumexp(next_dist_log_prob, dim=-1, keepdim=True)
        ...
        if self.add_noise:
            l_R_final = (l_R_final - p_dict["translation"]) @ torch.t(p_dict["rotation"])
