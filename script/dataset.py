...
class DataProcessor(object):
    def __init__(self, args, mode="train"):
        ...
        self.t_Z[:, args.num_ligand_atom_feature - 1] = 1.0  # termination
        self.f_Z = torch.zeros(1, args.num_ligand_atom_feature)  # focus
        self.f_Z[:, args.num_ligand_atom_feature - 1] = 1.0  # focus

        self.distance_expand1 = SoftOneHot(*args.dist_one_hot_param1, gamma=args.gamma1)
        self.distance_expand2 = SoftOneHot(
            *args.dist_one_hot_param2, gamma=args.gamma2, normalize=True
        )
    ...
    def get_random_traj(
        ...
        while i == 0 or torch.sum(avail) > 0:
            if i == 0:
                f_R = self.o_R
                p_ind = torch.topk(
                    torch.cdist(self.o_R, p_dict["p_R"]),
                    k=self.args.k,
                    dim=-1,
                    largest=False,
                )[1][0]
            ...
            if i == 0:
                ...
                if torch.sum(adj[cur]) == 0:  # No neighbors, dead end
                    ...
                    data.dist_ll_output = self.distance_dummy.repeat(
                        i + self.num_token, 1
                    )
    ...
    def get_input_from_data(self, data):
        ...
        l_n = l_Z.shape[0]
        l_dict = {"l_Z": l_Z, "l_R": l_R, "l_n": l_n, "l_adj": torch.Tensor(ligand_adj)}
        ...
        p_dict = {"p_Z": p_Z, "p_R": p_R, "p_n": p_n, "p_cond": pocket_prop}

        if self.use_scaffold:
            l_dict["s_n"] = scaff_n

        # Edge indices
        p_knn_index = torch.topk(
            torch.cdist(l_R, p_R), k=self.args.k, dim=-1, largest=False
        )[1]
        p_index = p_knn_index.view(-1)
        l_index = (
            torch.LongTensor(list(range(l_n)))
            .unsqueeze(-1)
            .repeat(1, self.args.k)
            .view(-1)
        )
...
class PDBbindDataset(MoleculeDataset):
    ...
    def __getitem__(self, idx):
        ...
        data_dict = {"whole": whole, "traj": traj, "key": key}
        return data_dict
